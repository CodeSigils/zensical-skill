...
This is a compact runtime reference for current primary sources. Each row must
...
| Admonitions | [authoring/admonitions](https://zensical.org/docs/authoring/admonitions/) | Checked 2026-09-26 | Supported callout types, nesting, collapsible details, and configuration. GitHub-style `> [!NOTE]` callouts are also accepted since 0.0.62. Confirm against the target version. |
| Content tabs | [authoring/content-tabs](https://zensical.org/docs/authoring/content-tabs/) | Checked 2026-09-26 | Tab syntax, nested content, anchors, and linked tabs; labels link by title rather than position, and matching labels link across the whole site rather than one page. |
| Code-block line anchors | [authoring/code-blocks](https://zensical.org/docs/authoring/code-blocks/) | Checked 2026-09-11; version-sensitive | The upstream example enables `pymdownx.highlight.anchor_linenums`. Inspect rendered target output before keeping or disabling it; replacing the extension table requires preserving all required defaults. |
...
| Customization and templates | [customization](https://zensical.org/docs/customization/) | Checked 2026-09-09; version-sensitive | Additional CSS/JavaScript, `custom_dir`, MiniJinja templates, focused block overrides, and custom page templates; compare against the installed release before editing. |
| Front matter and landing pages | [authoring/frontmatter](https://zensical.org/docs/authoring/frontmatter/) | Checked 2026-09-09; version-sensitive | Page-selected templates, metadata, hidden elements, and homepage conventions; inspect the site's actual landing-page source. |
...
| Language selection | [setup/language](https://zensical.org/docs/setup/language/) | Checked 2026-09-13 | A generated project has one canonical document language. Alternate-language links may point at other paths, domains, or subdomains and carry `hreflang`; verify each deployment's base path and build arrangement. This is not Zola-style translated-content routing. |
| Sitemaps and canonical URLs | [setup/navigation](https://zensical.org/docs/setup/navigation/) and [create-your-site](https://zensical.org/docs/create-your-site/) | Checked 2026-09-13 | Navigation features such as instant navigation rely on a configured `site_url` and generated `sitemap.xml`. Inspect generated URLs and sitemap entries during an explicit release review; neither proves hosting, DNS, indexing, nor deployment. |
| Feeds / RSS | [MkDocs plugin compatibility](https://zensical.org/docs/compatibility/mkdocs/plugins/) | Checked 2026-09-26 | Supported natively since 0.0.65 as the `rss` plugin, producing RSS 2.0 and JSON Feed. It is opt-in, so a site with no plugin table generates no feed, and no discovery link is emitted, so a theme override is required for readers to find it. Zensical ignores `cache_dir` and `use_material_social_cards`. There is no dedicated upstream feeds guide; verify accepted settings against the installed version, because unknown settings are rejected. |
...
| Release channel and upgrade boundary | [upgrade guide](https://zensical.org/docs/upgrade/) and [upcoming changes](https://zensical.org/upcoming-changes/) | Checked 2026-09-26; the project states it is on `0.0.x` alpha/development releases and will move to `0.x` at a beta, with `0.1.0` announced for 2026-11-05 | The pre-`1.0` channel means syntax and defaults may change without a deprecation cycle. Recheck these pages before recommending an upgrade, and treat behavior verified on an earlier `0.0.x` release as needing revalidation rather than as current. |
